...
shellout.write("\
echo \"Ril\tTag\tHits\">" + wdir + "/Analysis/hitcount.lst && \
echo \"Ril\tTagPos\tTag\tPos\tCall\">" + wdir + "/Analysis/snpreport.lst && \
\
cat " + wdir + "/Analysis/Genotypes/*/*.hits.txt|sort -u >>" + wdir + "/Analysis/hitcount.lst &&  \
R --slave --vanilla <" + wdir + "/hpcjcl/rhits.R & \
\
cat " + wdir + "/Analysis/Genotypes/*/*.snps.txt |sort -u >>" + wdir + "/Analysis/snpreport.lst && \
R --slave --vanilla <" + wdir + "/hpcjcl/rsnps.R && \
\
sed 's#NA#\"-\"#g' -i " + wdir + "/hpcjcl/SNPreport.txt && \
\
head -1 " + wdir + "/hpcjcl/SNPreport.txt>" + wdir + "/hpcjcl/snpheader && \
\
sed '1d' -i " + wdir + "/hpcjcl/SNPreport.txt && \
\
echo \"Chromosome\tChromo_arm\tcM\">" + wdir + "/hpcjcl/chromohead && \
echo \"Tag\" >" + wdir + "/hpcjcl/taghd && \
paste " + wdir + "/hpcjcl/taghd " + wdir + "/hpcjcl/snpheader " + wdir + "/hpcjcl/chromohead>" + wdir + "/hpcjcl/finalheader && \
cut -d\\\" -f2 " + wdir + "/hpcjcl/SNPreport.txt|cut -d_ -f1|sort >" + wdir + "/hpcjcl/foundsnps && \
cut -f1 " + wdir + "/hpcjcl/ChromoLocations|sort>" + wdir + "/hpcjcl/chromosnps && \
comm -1 -2 " + wdir + "/hpcjcl/chromosnps " + wdir + "/hpcjcl/foundsnps >" + wdir + "/hpcjcl/greplist && \
LC_ALL=C grep -w -f " + wdir + "/hpcjcl/greplist " + wdir + "/hpcjcl/ChromoLocations|sort -k1,1 >" + wdir + "/hpcjcl/commonlocations && \
cut -d\\\" -f2 " + wdir + "/hpcjcl/SNPreport.txt|cut -d_ -f1>" + wdir + "/hpcjcl/snptagnames && \
paste " + wdir + "/hpcjcl/snptagnames " + wdir + "/hpcjcl/SNPreport.txt|sort -k1,1 >" + wdir + "/hpcjcl/tmpreport1 && \
join -a1 " + wdir + "/hpcjcl/tmpreport1 " + wdir + "/hpcjcl/commonlocations|tr ' ' '\\t' > " + wdir + "/hpcjcl/FinalSNPs && \
cat " + wdir + "/hpcjcl/finalheader " + wdir + "/hpcjcl/FinalSNPs>" + wdir + "/hpcjcl/SNPreport.txt && \
sed 's#\"##g' -i " + wdir + "/hpcjcl/SNPreport.txt && \
perl " + wdir + "/hpcjcl/shufflecols.pl && sed 's#\\r##' -i " + wdir + "/Analysis/SNPreport.txt")
...
#remove medium files: remove hpcjcl
...
print "Total runtime = " + str(end_time - time2 + time1 - start_time) + "\n"
